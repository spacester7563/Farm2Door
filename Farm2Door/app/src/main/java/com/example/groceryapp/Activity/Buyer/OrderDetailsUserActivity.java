        ...
        reference.child(orderTo).child("Orders").child(orderId).child("Items")
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        String[] itemIdArray = itemIdList.toArray(new String[itemIdList.size()]);

                        // Save the array to SharedPreferences
                        SharedPreferences sharedPreferences = getSharedPreferences("MyPrefs", Context.MODE_PRIVATE);
                        SharedPreferences.Editor editor = sharedPreferences.edit();
                        editor.putInt("itemIdArraySize", itemIdArray.length);
                        for (int i = 0; i < itemIdArray.length; i++) {
                            editor.putString("itemIdArray_" + i, itemIdArray[i]);
                        }
                        editor.apply();
