...
public class AdapterProductUser extends RecyclerView.Adapter<AdapterProductUser.HolderProductUser> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull HolderProductUser holder, int position) {
        ...
        holder.SubscribeTv.setOnClickListener(view -> {
            String[] time = {"Weekly", "Monthly", "Quarterly", "Yearly"};
            ...
            builder.setItems(time, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (which == 0){
                        ...
                        editor.putString("pid", modelProduct.getProductID());
                        editor.apply();

                        Toast.makeText(context, "Subscribed", Toast.LENGTH_SHORT).show();
                        holder.SubscribeTv.setVisibility(View.GONE);
                    }else if (which == 1){
                        SharedPreferences sharedPreferences = context.getSharedPreferences("MyPrefs", Context.MODE_PRIVATE);
                        SharedPreferences.Editor editor = sharedPreferences.edit();
                        editor.putString("pid", modelProduct.getProductID());
        ...
        SharedPreferences sharedPreferences = context.getSharedPreferences("MyPrefs", Context.MODE_PRIVATE);
        String savedPid = sharedPreferences.getString("pid", ""); // -1 is the default value if the key doesn't exist
        if (savedPid == modelProduct.getProductID()) {
            // The PID exists in shared preferences
            holder.SubscribeTv.setVisibility(View.GONE);
        } else {
            // The PID doesn't exist in shared preferences or it's different from the current PID
        }
    ...
    class HolderProductUser extends RecyclerView.ViewHolder{
        ...
        private TextView discountedNote,title,discountPrice,originalPrice,addToCart,SubscribeTv;
